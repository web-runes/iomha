...
} from "../utils/redirectValidation.js";
...
export async function revalidateRemoteImage(
	...
	return {
		...
		etag:
			res.headers.get("Etag") ?? (res.ok ? undefined : revalidationData.etag),
		lastModified:
			res.headers.get("Last-Modified") ??
			(res.ok ? undefined : revalidationData.lastModified),
	};
}

function webToCachePolicyRequest({
	url,
	method,
	headers,
...
function webToCachePolicyResponse({
	status,
	headers,
