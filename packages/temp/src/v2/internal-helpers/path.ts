...
export function collapseDuplicateLeadingSlashes(path: string): string {
	if (!path) {
		return path;
	}
...
export function collapseDuplicateSlashes(path: string): string {
	if (!path) {
		return path;
	}
...
export const MANY_TRAILING_SLASHES: RegExp = /\/{2,}$/g;

export function collapseDuplicateTrailingSlashes(
	path: string,
...
export function removeLeadingForwardSlash(path: string): string {
...
function startsWithDotDotSlash(path: string): boolean {
	const c1 = path[0];
	const c2 = path[1];
	const c3 = path[2];
	return c1 === "." && c2 === "." && c3 === "/";
...
function startsWithDotSlash(path: string): boolean {
	const c1 = path[0];
	const c2 = path[1];
	return c1 === "." && c2 === "/";
}
 ...
 */
export function isParentDirectory(
	parentPath: string,
	childPath: string,
): boolean {
